...
const Validator = require('./variant-six_modules/validator.class');
const DataTransformation = require('./variant-six_modules/dataTransformation.class');
...
const actionHandler = async () => {
    ...
    if (action !== 'minimumNumber' && action !== 'formulaAsString') {
      process.stderr.write(
        `${errorMessage} \"Action must be included in the given list ['formulaAsString', 'minimumNumber'] :(\"\n`
      ); 
      process.exit(1);
    }

    Validator.isEmpty(input) && process.stdout.write('Enter the text and press ENTER to formulaAsString/minimumNumber | press CTRL + C to exit: ')
...
program
  ...
  .requiredOption('-a --action <action>', 'An action formulaAsString/minimumNumber')
